...
| D-12 | Visa Processing Letter: RM approval removed from DOCS flow. DOCS target flow: DV/EDV (India Team specialist) verifies document → manually sends to employee via "Send Document". Comments field required (max 300 chars, one line). *(Under review — Satish Malla confirmed at India SME meeting 14.05.2026 that RM approval IS required for Visa Processing Letter. D-12 reflects DOCS platform design intent; Satish's confirmation contradicts it. Development of Track B blocked pending OQ-15 resolution.)* | Darya Taranda (Reference Letters KB page) | 2026-05-06 | 01, 02 |
...
| OQ-15 | **Track B letters (Visa Processing, LOR) — RM approval solution required.** Satish Malla confirmed at India SME meeting (14.05.2026) that RM approval is a mandatory step in the current Assist process for both Visa Processing Letter and LOR (contradicting D-12 and D-13). The DOCS platform does not currently support routing a document to an RM for approval. Development of Track B is blocked until a technical solution is identified and agreed. Options to investigate: (1) Employee enters RM name/email on the DOCS form; system sends verification email to RM; specialist verifies only after RM responds; (2) Specialist manually contacts RM outside DOCS and proceeds with verification after off-system RM confirmation; (3) Other DOCS platform capabilities (e.g., multi-step approval workflow). Investigation required with DOCS platform team (Olga Chaban) and India Team (Satish). | Dzmitry Fedarovich / Olga Chaban / Satish Malla | 2026-05-14 | DOCS platform investigation / Stage 3 | **Open — blocking Track B development** |
